<div class="max-w-7xl mx-auto">
    <div class="relative rounded-xl overflow-hidden mb-6 shadow-md">
        <img src="https://placehold.co/1200x300/fde68a/0a0a0a?text=Car+Types" alt="Car Types" class="w-full h-auto object-cover" onerror="this.onerror=null;this.src='https://placehold.co/1200x300/cccccc/ffffff?text=Image+Not+Found'">
    </div>

    <div class="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
        @forelse($types as $type)
            <div wire:key="ctype-{{ $type->id }}" class="border border-gray-200 dark:border-gray-700 hover:border-primary flex flex-col bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 overflow-hidden">
                @php $canManage = auth()->check() && ($type->user_id === auth()->id() || optional(auth()->user()->role)->slug === 'admin'); @endphp

                <a wire:navigate href="{{ route('web.cars', $type->id) }}" class="p-4 flex-grow flex flex-col items-center justify-center text-center">
        ...
        @empty
            <div class="flex items-center justify-center col-span-full p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm">
                <span class="text-gray-500 dark:text-gray-400">No car types available</span>
            </div>
        @endforelse
    </div>
    ...
    <!-- Create Type Modal -->
    <x-modal name="create-car-type" :show="false" maxWidth="md" focusable>
        <div class="p-6">
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Add New Car Type</h2>
            <form wire:submit.prevent="createType" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Name</label>
                    <input type="text" wire:model.defer="name" class="mt-1 w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 focus:border-primary focus:ring-primary" placeholder="e.g., Sedan, SUV, Microbus">
    ...
    <!-- Edit Type Modal -->
    <x-modal name="edit-car-type" :show="false" maxWidth="md" focusable>
        <div class="p-6">
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Edit Car Type</h2>
    ...
    <x-modal name="delete-car-type" :show="false" maxWidth="sm" focusable>
        <div class="p-6">
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">Delete Car Type</h2>
