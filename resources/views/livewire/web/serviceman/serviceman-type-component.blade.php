<div class="max-w-7xl mx-auto">
    <x-sponsor wire:ignore  title="serviceman-type"/>

    <div class="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
        @forelse($types as $type)
            <div wire:key="stype-{{ $type->id }}" class="border border-gray-200 dark:border-gray-700 hover:border-primary flex flex-col bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 overflow-hidden">
                @php $canManage = auth()->check() && ($type->user_id === auth()->id() || optional(auth()->user()->role)->slug === 'admin'); @endphp

                <a wire:navigate href="{{ route('web.servicemen', $type->id) }}" class="p-4 flex-grow flex flex-col items-center justify-center text-center">
                ...
                @if($canManage)
                    <div class="border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 p-2 flex items-center justify-center gap-2">
                        ...
                        @endcan
                        @can('app.serviceman_types.delete')
        ...
        @empty
            <div class="flex items-center justify-center col-span-full p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm">
                <span class="text-gray-500 dark:text-gray-400">No serviceman types available</span>
            </div>
        @endforelse
    </div>
    ...
    <!-- Create Type Modal -->
    <x-modal name="create-serviceman-type" :show="false" maxWidth="md" focusable>
        <div class="p-6">
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Add New Serviceman Type</h2>
            <form wire:submit.prevent="createType" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Name</label>
                    <input type="text" wire:model.defer="name" class="mt-1 w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 focus:border-primary focus:ring-primary" placeholder="e.g., Electrician, Plumber">
                ...
                <div class="mt-6 flex items-center justify-end gap-3">
                    <button type="button" class="px-4 py-2 rounded-md border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700" @click="$dispatch('close-modal', 'create-serviceman-type')">Cancel</button>
    ...
    <!-- Edit Type Modal -->
    <x-modal name="edit-serviceman-type" :show="false" maxWidth="md" focusable>
        <div class="p-6">
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Edit Serviceman Type</h2>
            <form wire:submit.prevent="updateType" class="space-y-4">
                ...
                <div class="mt-6 flex items-center justify-end gap-3">
                    <button type="button" class="px-4 py-2 rounded-md border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700" @click="$dispatch('close-modal', 'edit-serviceman-type')">Cancel</button>
    ...
    <x-modal name="delete-serviceman-type" :show="false" maxWidth="sm" focusable>
        <div class="p-6">
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">Delete Serviceman Type</h2>
            <p class="text-sm text-gray-600 dark:text-gray-400">Are you sure you want to delete this type?</p>
            <div class="mt-6 flex items-center justify-end gap-3">
                <button type="button" class="px-4 py-2 rounded-md border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700" @click="$dispatch('close-modal', 'delete-serviceman-type')">Cancel</button>
