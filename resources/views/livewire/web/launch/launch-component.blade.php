<div class="max-w-7xl mx-auto">
                ...
                <div class="p-5 flex-grow">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            @if($launch->details)
                                <p class="text-sm text-gray-600 dark:text-gray-300 mt-1 line-clamp-2">{{ $launch->details }}</p>
                            @endif
                        </div>
                    </div>
                </div>
    ...
    @auth
        <button wire:click="openLaunchForm" class="fixed bottom-6 right-6 h-14 w-14 bg-primary text-white rounded-full flex items-center justify-center shadow-lg hover:bg-primary/90 transition z-30" aria-label="Add Launch">
            <i class='bx bx-plus text-3xl'></i>
        </button>
    @endauth
    @guest
        <a href="{{ route('login') }}" class="fixed bottom-6 right-6 h-14 w-14 bg-primary text-white rounded-full flex items-center justify_center shadow-lg hover:bg-primary/90 transition z-30" aria-label="Login to add launch">
        ...
        <div class="p-6">
            ...
            <form wire:submit.prevent="{{ $selectedId ? 'updateLaunch' : 'createLaunch' }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea rows="3" wire:model.defer="details" class="mt-1 w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 focus:border-primary focus:ring-primary" placeholder="Launch schedule, stops, etc."></textarea>
