<div  class="m-0 md:m-2 capitalize" x-data="category()">
    <div class="flex justify-between gap-4 mb-2 capitalize">
        <p class="text-gray-700 dark:text-gray-200 text-xl font-semibold">@lang("all categories")</p>
        <div class="flex text-sm gap-1">
            ...
            <span class="text-gray-500 dark:text-gray-300">@lang("categories")</span>
        </div>
    </div>
    <main class="h-full">
        @can("app.categories.create")
            <div class="sm:flex sm:items-center sm:justify-between">
                <div>
                    <div class="flex items-center gap-x-3">
                        <h2 class="text-lg font-medium text-gray-800 dark:text-white">@lang('categories')</h2>

                        <span class="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full dark:bg-gray-800 dark:text-blue-400">@lang('total'): {{ $categories->total() }}</span>
                    </div>
                </div>
                        ...
                        <div class="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg">
                            <table class="border border-collapse min-w-full">
                                ...
                                <tr class="text-center object-cover items-center h-10 text-nowrap">
                                    ...
                                    <x-field>@lang('parent')</x-field>
                                    <x-field>@lang('children')</x-field>
                                ...
                                @forelse($categories as $category)
                                    <tr wire:key="{{$category->id}}"  id="item-id-{{$category->id}}" class="text-gray-700 dark:text-gray-300 capitalize text-center object-cover items-center" :class="{'bg-gray-200 dark:bg-gray-600': rows.includes('{{$category->id}}') }">
                                        <td class="max-w-48 truncate pl-2">
                                            @can("app.categories.delete")
                                                <input x-model="rows" value="{{$category->id}}" id="{{ $category->id }}" type="checkbox" class="justify-between text-blue-500 border-gray-300 rounded dark:bg-darker dark:ring-offset-gray-900 dark:border-gray-500">
                                            @endcan
                                        </td>

                                        <td class="max-w-48 truncate px-4 text-sm font-medium text-gray-700 whitespace-nowrap">
{{--                                            <a href="{{route('category', $category->slug)}}" wire:navigate class="flex justify-start gap-x-3">--}}
                                                <div class="flex items-center gap-x-2">
                                                    <span class="w-10 h-10 rounded-full bg-purple-600 border dark:border-gray-600 shadow-xl overflow-hidden flex items-center justify-center flex-shrink-0">
                                                        <img src="{{ getImage($category, 'profile', 'thumb') }}" alt="" onerror="{{getErrorImage()}}" class="w-full h-full object-cover">
                                                    </span>
                                                    <h2 class="font-medium text-gray-800 dark:text-white flex-grow">{{ $category->name }}</h2>
                                                </div>
{{--                                            </a>--}}
                                        </td>
                                        ...
                                        <td class="max-w-48 truncate px-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">
                                            ...
                                            @else
                                                <p><strong>Parent:</strong> None (This is a root category)</p>
                                            @endif
                                        ...
                                        <td class="max-w-48 truncate px-12 text-sm font-medium text-gray-700 whitespace-nowrap">
                                            <div
                                                ...
                                                <button type="button" wire:click="changeStatus({{ $category->id }})" class="cursor-pointer text-sm font-normal {{ $category->status=='published'?'text-emerald-500':'text-pink-500' }} ">{{ $category->status }}</button>
                                            </div>
                                        </td>
                                        <td class="max-w-48 truncate px-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">{{ str($category->updated_at)->words(5) }}</td>

                                        <td class="max-w-48  truncate px-4 text-sm whitespace-nowrap">
                                            <div class="flex justify-between text-center gap-x-6">
                                                @can("app.categories.edit")
                                                    <button @click="editModal('{{$category->id}}')"
                                                    >
                                                ...
                                                @can("app.categories.delete")
                                                    <button
                                                        @click.prevent="$dispatch('delete', { title: 'Are you sure to delete', text: 'It is not revertable', icon: 'error',actionName: 'deleteSingle', itemId: {{$category->id}} })"
                                                    >
                ...
                <form @submit.prevent="editMode? $wire.editData(): $wire.saveData()"
                    ...
                    <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                        <div>
                            ...
                            <x-text-input errorName="name" errorName="name" x-ref="inputName" id="name" wire:model.live.debounce.1000ms="name" type="text"/>
                        </div>
                        <div>
                            <label class="text-gray-700 dark:text-gray-200" for="description">@lang('description')</label>
                            <x-text-input errorName="description" wire:model="description" type="text"/>
                        </div>
                        ...
                        @if ($c && $c->children->isEmpty())
                            <div>
{{$c->children}}
                                <label class="text-gray-700 dark:text-gray-200" for="type">@lang('parent category')</label>
                                <x-select errorName="parent_id" id="parent_id" wire:model="parent_id">
                                    <option value="">@lang('select category')</option>
                                    @foreach($selectCategories as $i => $cat)
